...
def human_move_and_click(page, locator, delay_before=2):
    ...
    try:
        if locator.is_visible(timeout=5000):
            ...
            if box:
                # Add randomness to the target point
                target_x = box['x'] + box['width'] / 2 + random.uniform(-5, 5)
                target_y = box['y'] + box['height'] / 2 + random.uniform(-5, 5)
                
                # Move steps for human-like speed
                page.mouse.move(target_x, target_y, steps=30)
                time.sleep(0.5)
...
def run_application_loop():
    ...
    with open(csv_file, mode='r', newline='', encoding='utf-8') as f:
        ...
        for row in reader:
            url = row.get('url') or row.get('link') or (list(row.values())[0] if row else None)
        ...
        try:
            ...
            for index, url in enumerate(links):
                                            ...
                                            if human_move_and_click(page, panel_btn, delay_before=1):
                                                ...
                                                for frame in page.frames:
                                                    for c_sel in close_selectors:
                                                        try:
                                                            c_btn = frame.locator(c_sel).first
                                                            if c_btn.is_visible(timeout=500):
                                                                print(f"Found close button in frame '{frame.name or 'iframe'}': {c_sel}")
                                                                if human_move_and_click(page, c_btn, delay_before=1):
                                                ...
                                                for sub_sel in submit_selectors:
                                                    try:
                                                        ...
                                                        if sub_btn.is_visible(timeout=5000):
                                                            ...
                                                            # Explicitly scroll into view to avoid being obscured
                                                            ...
                                                            if human_move_and_click(page, sub_btn, delay_before=2):
                                                                ...
                                                                print("SUCCESS: Submit Application button clicked via human interaction!")
                                                                clicked_submit = True
                                                                break
                                                            else:
                                                                # Fallback to force click if human click fails or is obscured
                                                                print("Human click failed or element obscured. Attempting force click...")
                                                                sub_btn.click(force=True)
                                                                link_analytics["submit_clicked_time"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                                                                print("SUCCESS: Submit Application button force-clicked!")
                                                                clicked_submit = True
                                                                break
                                                    except: continue
                                                ...
                                                if clicked_submit:
                                                    print("Waiting 10s for submission to process...")
                                                    time.sleep(10)
                ...
                try:
                    ...
                    # Save analytics incrementally to current directory
